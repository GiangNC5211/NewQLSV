...
export class SinhvienService {
  ...
  students$$ = new BehaviorSubject<StudentData[]>([]);
  students$ = this.students$$.asObservable().pipe(
    tap(list => this._students = list)
  );

  add$$ = new Subject<StudentData>();
  add$ = this.add$$.asObservable().pipe(
    tap(student => this.addStudentHandler(student))
  );

  remove$$ = new Subject<number>();
  remove$ = this.remove$$.asObservable().pipe(
    tap(id => this.removeStudentHandler(id))
  )

  update$$ = new Subject<{id: number, student: StudentData}>();
  update$ = this.update$$.asObservable().pipe(
    tap(event => {
  ...
  getStudentById(id: number) {
    return this.students.find(student => student.STT = id);
  }
  ...
  addStudentHandler(student: StudentData) {
    ...
    this.students$$.next([...this.students, studentWithId]);
  }
